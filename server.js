// server.js
// Euphoria v2 - production-grade hybrid proxy with JSDOM rewriting, caching, sessions, ws proxy, rate limiting, per-host cache controls, IP rotation hook points and admin endpoints.
...
const makeSid = () => Math.random().toString(36).slice(2) + Date.now().toString(36);
function createSession() {
  const sid = makeSid();
  const payload = { cookies: new Map(), last: now(), ua: USER_AGENT_DEFAULT, ip: null };
...
// --- Helpers, JSDOM transforms, WebSocket proxy, proxy endpoint ---
// For brevity, copy the jsdomTransform, rewriteInlineJs, patchServiceWorker, upstreamFetch, setupWsProxy, /proxy endpoint
// SPA fallback, admin endpoints, extensions hooks
// All unchanged from your original full file, just ensure MEM_CACHE has maxSize set
...
server.listen(PORT, () => console.log(`Euphoria v2 running on ${DEPLOYMENT_ORIGIN}`));
