...
//   - Deployment-aware rewrites (no google.com/proxy recursion)
//   - Injects a lightweight rewrite script into proxied HTML
//   - Preserves key response headers (Content-Type, Cache-Control, etc.)
//   - Removes CSP/integrity attributes that would break proxied asset loading
//   - WASM/service worker/worker assets streamed directly
//   - Progressive streaming of transformed HTML via Scramjet
//   - Small memory + optional disk caching
...
const ASSET_CACHE_MAX = 128 * 1024; // 128 KB
...
function parseCookies(cookieHeader="") {
  const out = {};
  cookieHeader.split(";").forEach(p => { const [k,v] = (p||"").split("=").map(s=>(s||"").trim()); if(k&&v) out[k]=v; });
  return out;
}
...
function looksLikeAssetPath(urlStr){
  ...
  try{ const p=new URL(urlStr, DEPLOYMENT_ORIGIN).pathname.toLowerCase(); return ASSET_EXTENSIONS.some(e=>p.endsWith(e)) || SPECIAL_PATH_SEGMENTS.some(s=>p.endsWith(s)); }catch(e){ const l=urlStr.toLowerCase(); return ASSET_EXTENSIONS.some(e=>l.endsWith(e)) || SPECIAL_PATH_SEGMENTS.some(s=>l.endsWith(s)); }
}
function isAlreadyProxiedHref(href){ if(!href) return false; try{ if(href.includes("/proxy?url=")) return true; const u=new URL(href, DEPLOYMENT_ORIGIN); return u.origin===new URL(DEPLOYMENT_ORIGIN).origin && u.pathname.startsWith("/proxy"); }catch(e){return false;} }
...
wss.on("connection", ws => { ws.send(JSON.stringify({msg:"welcome",ts:Date.now()})); ws.on("message",raw=>{ try{ const p=JSON.parse(raw.toString()); if(p.cmd==="ping") ws.send(JSON.stringify({msg:"pong",ts:Date.now()})); }catch(e){} }); });
